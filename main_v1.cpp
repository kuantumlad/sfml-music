...
// this version agrees with the DFT results of this signal from MATHEMATICA!
...
int main(){
  ...
  double frequency_test = 5.0;
  
  
  for (unsigned i = 0; i < SAMPLES; i++) {
    raw[i] = 100.0 * sin(frequency_test*x);//*TWO_PI);
    x += increment;
  ...
  */
  ...
  trueFreqLines.append( sf::Vector2f(4.0*frequency_test, 0) );
  trueFreqLines.append( sf::Vector2f(4.0*frequency_test, windowY) );
  ...
  while(window.isOpen()){
	...
	temp_bufferLines.append( sf::Vector2f( b, windowY/2 + (int)norm_sample_amp) );
	current_buffer.push_back(norm_sample_amp);
	b++;
      }

      for( int t1 = 1; t1 < current_buffer.size()/2 ; t1++ ){
	if( t1 >= current_buffer.size()/2 ) continue;
	double N = current_buffer.size()/2.0;
	double sum_real = 0.0;
	double sum_img = 0.0;	
	for( int t2 = 1; t2 < current_buffer.size()/2; t2++ ){
	...
	temp_freqLines.append( sf::Vector2f(t1, windowY/2 - amp) );
      }
