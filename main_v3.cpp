...
int main(){
  ...
  if( !buffer.loadFromFile("/Users/bclary/Documents/work/codeprojects/music/A.wav") ){
  ...
  double freq = 45; // choose frequency between 0 to 1000. 
  ...
  */
  ...
  sf::Music music;
  //if (!music.openFromFile("/home/kuantumlad/Documents/projects/sfml-music/A.wav")){
  if( !music.openFromFile("//Users/bclary/Documents/work/codeprojects/music/A.wav")) {
  ...
  while(window.isOpen()){
	...
	b++;
      ...
      for( int t1 = 0; t1 < SAMPLES/2; t1++ ){//current_buffer.size(); t1++ ){
	if( t1 >= current_buffer.size()/2 ) continue;
	double N = current_buffer.size();
	double sum_real = 0.0;
	double sum_img = 0.0;	
	for( int t2 = 0; t2 < SAMPLES; t2++){//current_buffer.size(); t2++ ){
